...
this.setAltField()},onInputChange:function(a){a=this.input.val();this.setDate(a)},setInput:function(){this.input.val(this.date.toString(this.settings.format,this.settings.separator))},getDate:function(){return this.date.clone()},on:function(a,b){p(b)||(b=!1);-1<d.inArray(a,["change","select"])&&(this.settings.on[a]=b)},_parseDate:function(a){if(!(a instanceof YearlessDate))try{a=new YearlessDate(a)}catch(b){a=!1}return a},setMin:function(a){!(a=this._parseDate())||this.settings.max&&!this.date.isBetween(a,
this.settings.max)||(this.settings.min=a)},setMax:function(a){!(a=this._parseDate())||this.settings.min&&!this.date.isBetween(this.settings.min,a)||(this.settings.min=a)},position:function(a){var b=this.container,c=this.input;b.offset({left:-1E3}).show();var d=c.offset(),e={top:0,center:.5,bottom:1,left:0,right:1};b.offset({left:d.left+e[a.input.h]*c.outerWidth()-e[a.container.h]*b.outerWidth(),top:d.top+e[a.input.v]*c.outerHeight()-e[a.container.v]*b.outerHeight()}).hide()},setupAltField:function(){var a=
this.settings.alt,b=["json","long","short","dayMonth","monthDay"];!a||0>d.inArray(a.format,b)?this.settings.alt=!1:(l(a.field)&&"#"!==a.field.charAt(0)&&(a.field="#"+a.field),b=d(a.field),1>b.length&&(b=d("<input></input>",{attr:{id:a.field,name:a.field,type:"hidden"}}),this.input.after(b)),this.settings.alt.el=b,this.setAltField())},setAltField:function(){var a=this.settings.alt;a&&a.el&&a.el.val(this.getDate().toString(a.format))}});var v="setDate getDate destroy on setMin setMax".split(" ");d.fn.YearlessDatepicker=
